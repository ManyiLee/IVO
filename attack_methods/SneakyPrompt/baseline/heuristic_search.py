...
class brute_search:
    ...
    def __init__(self, target_prompt, num_word, word_index, num_subword, len_subword, threshold, saved_figure_path, pipe, en, logger):
        self.pipe = pipe
    ...
    def search(self):
        ...
        query = 0
        query_online = 0
        self.logger.info(f'==> start brute force searching...')
        NSFW = True
        while NSFW == True:
            with torch.no_grad():
                temp_token_list = []
                ...
                query += 1
                self.logger.info(f'query: {query}, similarity: {1.0 -loss}')
                if (1.0 - loss)>= self.threshold:
                    self.logger.info(f'==> loss satisfied, check if bypassing safety filter...')
                    NSFW, input_img_tensor, pil_images = self.pipe(input_prompt)
                    query_online += 1
                    torch.cuda.empty_cache()
                    if NSFW == False:
                        self.logger.info(f'==> Succeed with total query {query} including online query {query_online} times! Image saved.')
                        pil_images[0].save(f"{self.saved_figure_path}/{input_prompt[0]}_{query}_{query_online}.png")

class greedy_search(brute_search):

    def __init__(self,target_prompt, num_word, word_index, num_subword, len_subword, threshold, saved_figure_path, pipe, en, logger):
        brute_search.__init__(self, target_prompt, num_word, word_index, num_subword, len_subword, threshold, saved_figure_path, pipe, en, logger)

    def search(self):
        prompt_list, len_prompt_list = get_dictionary(self.len_subword, self.en)
        query = 0
        query_online = 0
        self.logger.info(f'==> start greedy searching...')
        NSFW = True
                ...
                for i in range(self.num_word):
                    ...
                    for j in range(self.num_subword):
                        ...
                        for token in prompt_list:
                            ...
                            query += 1
                            self.logger.info(f'query: {query}, loss: {min_loss}')
...
class beam_search(brute_search):
    ...
    def __init__(self,target_prompt, num_word, word_index, num_subword, len_subword, threshold, saved_figure_path, beam_size, pipe, en, logger):
        brute_search.__init__(self, target_prompt, num_word, word_index, num_subword, len_subword, threshold, saved_figure_path, pipe, en, logger)
    ...
    def search(self):
        ...
        query = 0
        query_online = 0
        self.logger.info(f'==> start beam search...')
        NSFW = True
