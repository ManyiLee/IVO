...
tokenizer = CLIPTokenizer.from_pretrained("./pretrained_weight/clip-vit-base-patch32")
text_encoder = CLIPTextModel.from_pretrained("./pretrained_weight/clip-vit-base-patch32").to(torch_device)
metric = CLIPScore(model_name_or_path="./pretrained_weight/clip-vit-base-patch32").to(torch_device)
...
def get_dictionary(len_subword, en):
    if en == False:
        f = open('data/SneakyPrompt/vocab.json')
        ...
        f = open('data/SneakyPrompt/words-google-10000-english-usa-no-swears.json')
        data_json = json.load(f)
...
def get_nsfw_dl(target_prompt):
    ...
    tokenizer = AutoTokenizer.from_pretrained("./pretrained_weight/NSFW_text_classifier")
    model = AutoModelForSequenceClassification.from_pretrained("./pretrained_weight/NSFW_text_classifier")
...
def get_nsfw_word_index(target_prompt, n, add_one_more, add_num, logger):
    ...
    logger.info(f'The sensitive (Nudity and sexual content) words needing to be replaced are {nsfw_set}')
