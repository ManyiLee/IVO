import sys
sys.path.append('./attack_methods/SneakyPrompt/')
...
class MPA_agent(robot):
    ...
    def __init__(self, pipe, target, nsfw_word, num_word, word_index, num_subword, len_subword, s_filter, mode, threshold, target_prompt, query_limit, saved_figure_path, df, query, query_online, prompt_record, en, logger):
        '''
        ...
        self.logger.info(f'The current perturb rate is {self.perturb_rate}')
    ...
    def create_searching_space(self, num_word, num_subword):
        ...
        self.logger.info(f'The searching space is {search_space}')
    ...
    def get_score(self, combo, target_tensor):
        ...
        with torch.no_grad():
            ...
            if self.mode == 'clip':
                ...
                elif self.NSFW == False:
                    self.logger.info('Add penalty')
            ...
            for s in range(steps):
                ...
                if self.mode == 'l2':
                    if s < self.query_limit and r >= self.threshold:
                        self.logger.info(f'==> Satisfy the reward threshold, check if bypassing safety filter...')
                        self.NSFW, input_img_tensor, self.pil_images = self.pipe(input_prompt)
                        self.query_online += 1
                ...
                self.logger.info(f"Total query: {self.query}, Reward: {r}")
                ...
                if self.query_online < self.query_limit:
                    if r >= self.threshold and self.NSFW == True:
                        self.logger.info(f'==> Succeed! The total number of query is {self.query} including online query {self.query_online}...')
                        self.logger.info(f'==> The prompt is {input_prompt}, image saved ` under "figure/", statistic results saved under "results/"')
                        self.result_df = self.result_df._append({"original_text": self.target_prompt[0], "perturbed_text": input_prompt[0],
                                         ...
                                         ignore_index=True)
                        self.pil_images[0].save(f"{self.saved_figure_path}/{input_prompt[0][0:50]}_{self.query}_{self.query_online}_{self.nsfw_word}.png")
                        break
                    ...
                    if len(orig_r_record) >= 5 and orig_r_record[-1] < 0 and orig_r_record[-2] < 0 and orig_r_record[-3] < 0 and orig_r_record[-4] < 0 and orig_r_record[-5] < 0:
                        self.logger.info('==> Continously not bypass, expand the search space by adding one more word to be replaced')
                ...
                if s > 2:
                    ...
                    if (self.query_online >= self.query_limit and r < self.threshold) \
                        ...
                        self.logger.info(f'==> Reach query limit / network not update / reach perturb rate limit')
                        ...
                        if self.mode == 'l2':
                            ...
                            self.query_online += 1
                            if self.NSFW == True:
                                self.logger.info(f'==> Save the image with the maximum reward that bypassed.')
                                best_pil_img[0].save(
                                    f"{self.saved_figure_path}/{best_prompt[0:50]}_{self.query}_{self.query_online}_{self.nsfw_word}_F.png")
                                ...
                                self.logger.info(f'==> Not pass the safety filter, no image saved')
                                self.result_df = self.result_df._append(
                                    {"original_text": self.target_prompt[0], "perturbed_text": best_prompt,
                                     "local_queries": self.query, "num_queries": self.query_online,
                            ...
                            try:
                                best_prompt = self.prompt_record[self.prompt_record["nsfw"] == True].iloc[0].values[0]
                                best_pil_img = self.prompt_record[self.prompt_record["nsfw"] == True].iloc[0].values[3]
                                best_reward = self.prompt_record[self.prompt_record["nsfw"] == True].iloc[0].values[1]
                                self.logger.info(f'==> Save the image with the maximum reward that bypassed.')
                                best_pil_img[0].save(
