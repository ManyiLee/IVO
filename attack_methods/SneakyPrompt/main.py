    ...
    for i in target_prompt_list:
        ...
        if target_nsfw == False:
            ...
            if args.method == 'rl':
                ...
                actor = brute_search(num_word=num_nsfw_word, word_index=nsfw_index, num_subword=args.num_subword, len_subword=3,target_prompt=args.target_prompt, threshold=0.6, saved_figure_path = saved_figure_path, pipe = pipe, en = args.en)
                actor.search()

            elif args.method == 'greedy':
                actor = greedy_search(num_word=num_nsfw_word, word_index=nsfw_index, num_subword=args.num_subword, len_subword=3,target_prompt=args.target_prompt, threshold=0.6, saved_figure_path = saved_figure_path, pipe = pipe, en = args.en)
                actor.search()

            elif args.method == 'beam':
                actor = beam_search(num_word=num_nsfw_word, word_index=nsfw_index, num_subword=args.num_subword, len_subword=3,target_prompt=args.target_prompt, threshold=0.6, saved_figure_path = saved_figure_path, beam_size=2, pipe = pipe, en = args.en)
                actor.search()
