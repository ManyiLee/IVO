...
class SDPipeline():
    def __init__(self, device, mode="ti_sd", fix_seed=False):
        ...
        self.mode = mode
        self.model = StableDiffusionPipeline.from_pretrained("./pretrained_weight/stable-diffusion-v1-4", use_auth_token=True,
    ...
    def image_dogcat_check(self, pil_images):
        model = torch.load('./attack_methods/SneakyPrompt/model/animal_model_100.pth', map_location=torch_device)
        model.eval()
        translate = {
            "cane": "dog",
            "cavallo": "horse",
    ...
    def text_check(self, text_inputs):
        tokenizer = AutoTokenizer.from_pretrained("./pretrained_weight/NSFW_text_classifier", use_auth_token=True)
